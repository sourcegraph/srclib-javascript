...
function initNodeInfo(kind, typeInfo) {
    ...
    return {kind: nodeKind, typeInfo: nodeTypeInfo};
}
...
function analyseAll() {
    ...
    var searchVisitor = walk.make({
        ...
        MemberExpression: function(node, nodeInfo, c) {
            c(node.object, initNodeInfo(nodeInfo.kind));
            c(node.property, initNodeInfo("property"));
        },
        ObjectExpression: function(node, nodeInfo, c) {
            for (var i = 0; i < node.properties.length; ++i) {
                c(node.properties[i].value, initNodeInfo(nodeInfo.kind));
                c(node.properties[i].key, initNodeInfo(nodeInfo.kind));
            }
        ...
        Identifier: function(node, nodeInfo, c) {
            ...
            if (pathForDef === pathForId || ((nodeInfo.kind === "var") && (pathForDef !== pathForId))) {
                ...
                var defData = {
                    ...
                    Keyword: nodeInfo.kind,
                    Kind: typeFormat === "" ? nodeInfo.kind : typeFormat.kind,
                    Separator: " "
                };
                var def = {
                    Path: scopePathForId,
