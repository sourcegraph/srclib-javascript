...
function analyseAll() {
    ...
    var searchVisitor = walk.make({
        ...
        MemberExpression: function(node, nodeInfo, c) {
            ...
            if (currentClass && node.object.type == "ThisExpression") {
                var path;
                if (nodeInfo.left && scope.definer(currentClass, '+' + node.property.name)) {
                    // it's a setter
                ...
                } else if (!nodeInfo.left && scope.definer(currentClass, '-' + node.property.name)) {
                    // it's a getter
                    path = scope.getPath(node.property);
                    if (path) {
        ...
        CallExpression: function(node, kind, c) {
            ...
            if (node.callee.name === 'require') {
                emitModuleRef(node, node.arguments[0]);
            }
