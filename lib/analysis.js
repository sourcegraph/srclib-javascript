...
function emitModuleRef(node, moduleData) {
    console.error("moduleData = ", moduleData);
    var data = getDefinition(node.sourceFile, moduleData.end, moduleData.start);
...
function analyseAll() {
    ...
    var searchVisitor = walk.make({
        Function: function(node, nodeInfo, c) {
            if (node.id) {
                ...
                c(node.id, initNodeInfo("function", "(" + params.join(", ") + ")"));
            }

            for (var i = 0; i < node.params.length; ++i) {
                c(node.params[i], initNodeInfo("param"));
            }
            c(node.body, initNodeInfo("fn_body"));
        },
        ExportNamedDeclaration: function(node, nodeInfo, c) {
            //console.error("exported name declaration = ", node);
            for (var i = 0; i < node.specifiers.length; i++) {
                var spec = node.specifiers[i];
                //var data = getDefinition(node.sourceFile, spec.end, spec.start);
        ...
        VariableDeclaration: function(node, nodeInfo, c) {
            for (var i = 0; i < node.declarations.length; ++i) {
                ...
                c(decl.id, initNodeInfo("var"));
                if (decl.init) c(decl.init, initNodeInfo("var_init"));
            }
        },
        ...
        ObjectExpression: function(node, nodeInfo, c) {
            for (var i = 0; i < node.properties.length; ++i) {
                c(node.properties[i].value, initNodeInfo("object value"));
                c(node.properties[i].key, initNodeInfo("object property"));
            }
        },
    ...
    ternServer.files.forEach(function(file) {
        if (file.name.indexOf('node_modules') == -1) {
            ...
            // console.error("File ast = ", file.ast);
