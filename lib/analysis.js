...
function analyseAll() {
    ...
    var searchVisitor = walk.make({
        ...
        ExportNamedDeclaration: function(node, nodeInfo, c) {
            ...
            if (node.source) {
                emitModuleRef(node, node.source);
            }
        },
        ImportDeclaration: function(node, nodeInfo, c) {
            for (var i = 0; i < node.specifiers.length; ++i) {
                console.error("node specifier = ", node.specifiers[i].local.name);
            }

            console.error("source = ", node.source.value);
