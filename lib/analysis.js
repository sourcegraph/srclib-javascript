    ...
    var searchVisitor = walk.make({
        ...
        MemberExpression: function(node, nodeInfo, c) {
            ...
            if (currentClass && node.object.type == "ThisExpression") {
                var def, path;
                if (nodeInfo.left && (def = scope.definer(currentClass, '+' + node.property.name))) {
                    // it's a setter
                    path = scope.getPath(node.property);
                    if (path) {
                        processIdent(node, initNodeInfo(nodeInfo, "setter"), node.property.name, path);
                        return;
                    }
                } else if (!nodeInfo.left && (def = scope.definer(currentClass, '-' + node.property.name))) {
                    // it's a getter
                    path = scope.getPath(node.property);
                    if (path) {
                        processIdent(node, initNodeInfo(nodeInfo, "getter"), node.property.name, path);
                        return;
                    }
                }
            }
...
function processIdent(node, nodeInfo, name, path) {
    ...
    if (path) {
        pathForDef = path;
    } else if (typeof data.start == 'undefined') {
