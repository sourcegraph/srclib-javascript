...
function analyseAll() {
    ...
    var searchVisitor = walk.make({
        ...
        JSXElement: function(node, nodeInfo, c) {
            if (scope.getClass(node.openingElement.name.name)) {
                processIdent(node, nodeInfo, node.openingElement.name.name);
            }
            node.openingElement.attributes.forEach(function(attr) {
                c(attr.argument || attr.value, initNodeInfo(nodeInfo, "attr"));
            });
