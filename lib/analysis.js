...
module.exports.initTernServer = function(files) {
    ...
    ternServer.flush(function(err) {
        if (err) throw err;
        scope.mapLinesPathToScopePath(ternServer);
        //console.error("MAP = \n", scope.pathMap);
        data = analyseAll();
    });
    ...
    var searchVisitor = walk.make({
        ...
        ObjectExpression: function(node, kind, c) {
            for (var i = 0; i < node.properties.length; ++i) {
                //console.error("KEY = ", node.properties[i].key);
                //console.error("VAL = ", node.properties[i].value);
        ...
        Identifier: function(node, kind, c) {
            ...
            if (pathForDef === pathForId || ((kind === "var") && (pathForDef !== pathForId))) {
                //emit def here
                var scopePathForId = scope.pathMap[pathForId]
                ...
                var key = data.origin + "_" + data.start + "_" + data.end;
                var scopePathForDef = scope.pathMap[key];
                if (scopePathForDef === null || scopePathForDef === undefined) {
                    console.error("SOURCE FILE = ", node.sourceFile.name);
                    console.error("DATA = ", data);
                    console.error("External repo = ", externalRepo);
                    console.error("ERROR OCCURRED IN MAPPING SCOPE DEF PATH for", key, "NAME=", node.name);
                }
                var ref = {
