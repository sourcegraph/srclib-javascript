...
function emitModuleRef(moduleData) {
    ...
    //special case for sourcegraph processing
        ...
        ImportDeclaration: function(node, nodeInfo, c) {
            for (var i = 0; i < node.specifiers.length; ++i) {
                if (node.specifiers[i].local) {
                    ...
                    c(node.specifiers[i].local, initNodeInfo(nodeInfo, "import"));
                }

                if (node.specifiers[i].imported &&
                    node.specifiers[i].imported.name !== node.specifiers[i].local.name) {
                    var specifier = node.specifiers[i].imported;
                    var classDef = scope.getClass(specifier.name);
                    if (classDef) {
                        processIdent(specifier, nodeInfo, specifier.name, classDef.path);
                        continue;
                    }
                    c(node.specifiers[i].imported, initNodeInfo(nodeInfo, "import"));
                }
            }
...
function processIdent(node, nodeInfo, name, path) {
    ...
    if (pathForDef === pathForId || ((nodeInfo.kind === "var") && (pathForDef !== pathForId)) || ((nodeInfo.kind === "import") && !scopePathForDef)) {
        // Emit definition
        ...
        //Workaround for strange tern bug when for classes function types are returned
        if (nodeInfo.kind == "class" && typeFormat.kind == "function") {
            resKind = nodeInfo.kind;
        }
