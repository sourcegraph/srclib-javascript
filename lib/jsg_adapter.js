...
function convertSymbol(jsym) {
  ...
  if (strContains(sym.SymbolKey.Path, "/node_core_modules/")) {
    console.error("skipping symbol key path in node_core_modules:", sym.SymbolKey.Path);
    return;
  }
...
function makeTargetDefPath(t) {
  ...
  if (t.origin == "node" || t.namespace == "commonjs") {
    defPathKey.module = "lib/" + defPathKey.module + ".js";
  }
