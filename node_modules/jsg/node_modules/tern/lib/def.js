...
(function(mod) {
  ...
  var parseEffect = exports.parseEffect = function(effect, fn) {
    ...
    } else if (effect.indexOf("log ") == 0) {
      addEffect(fn, function(self, args, nodes) {
        var node = nodes[0] || node;
        if (!node) node = {sourceFile: {name: "unknown"}, start: 0, end: 0};
        if (!node.sourceFile) node = node.property;
        console.error("effect:", effect.slice(4), "at", node.sourceFile.name + ":" + node.start + "-" + node.end);
      });
    } else {
  ...
  function isSimpleAnnotation(spec) {
    ...
    for (var prop in spec)
      if (prop != "!type" && prop != "!doc" && prop != "!url" && prop != "!span" && prop != "!data" && prop != "!hint")
        return false;
    return true;
  }
  ...
  function passTwo(base, spec, path) {
    ...
    for (var name in spec) if (hop(spec, name) && name.charCodeAt(0) != 33) {
      ...
      if (typeof inner == "string") {
        ...
        if (inner["!hint"]) known.hint = inner["!hint"];
  ...
  function copyInfo(spec, type) {
    ...
    if (spec["!hint"]) type.hint = spec["!hint"];
  }
