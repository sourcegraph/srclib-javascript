...
(function(mod) {
  ...
  function canonicalType(types) {
    var arrays = 0, fns = 0, objs = 0, prim = null, hints = [];
    ...
    if (maxTp && hints.length) {
      for (var i = 0; i < hints.length; ++i) {
        var hint = hints[i];

        maxTp.doc = hint.doc;
        hint.forAllProps(function(prop, val, local) {
          if (local && prop != "<i>") {
            val.propagate(maxTp.defProp(prop));
          }
        });

        if (maxTp instanceof Fn) {
          maxTp.args = hint.args;
          maxTp.argNames = hint.argNames;
          maxTp.retval = hint.retval;
        }
        // TODO(sqs): apply type hints for arrays and prims
      }
    }
  ...
  var inferExprVisitor = {
    ...
    Identifier: ret(function(node, scope) {
      if (node.name == "arguments" && scope.fnType && !(node.name in scope.props)) {
        var args = scope.defProp(node.name, scope.fnType.originNode);
        args.fnArgs = true;
        args.addType(new Arr(scope.fnType.arguments = new AVal));
      }
